...
var app = new Vue({
    ...
    methods: {
        ...
        do_add_lingyong: function(id) {
            var title = id > 0 ? '资产领用' : '资产领用';
            var name = 'zclingyong_frm';
            var cansave = id > 0 ? false : true;
            var canedit = id > 0 ? false : true;
            var hideeditbtn = id > 0 ? false : true;
            ns.openWin(name, title, { canedit: canedit, canlingyong: cansave, id: id, hideeditbtn: hideeditbtn });
        }
    }
});
apiready = function() {
    ...
    app.form.OrderTypeID = ns.getPageParam('OrderTypeID') || 20;
