...
var app = new Vue({
    ...
    methods: {
        ...
        do_change_qty: function() {
            var that = this;
            if (that.limitCount <= 0) {
                return;
            }
            if (that.limitCount < that.form.Qty) {
                that.form.Qty = that.limitCount;
            }
        },
        do_save: function() {
            var that = this;
...
apiready = function() {
    ...
    app.sqzcid = ns.getPageParam('sqzcid') || 0;
    setTimeout(function() {
        app.get_data();
        api.sendEvent({
    ...
    api.addEventListener({
        ...
        if (ret) {
            if (ret.value) {
                app.form.EPCode = ret.value.epcode;
            }
        }
    });
    api.addEventListener({
